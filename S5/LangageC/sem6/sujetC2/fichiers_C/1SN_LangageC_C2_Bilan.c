...
void create(string *chaine_dest, char *chaine_src){
    // ****** TODO *******  
    char *new_chaine = malloc(sizeof(char)*(strlen(chaine_src)+1));
    if (new_chaine) {
        chaine_dest->str = new_chaine;
...
void add(string *chaine, char c){
    // ****** TODO *******
    char* nouv = realloc(chaine->str, (chaine->N+1) * sizeof(char));
    if (nouv) {
        nouv[chaine->N -1] = c;
        chaine->str = nouv;
        chaine->N ++;
...
void delete(string *chaine, int i){
    // ****** TODO *******  
    for (i; i<chaine->N; i++) {
        chaine->str[i] = chaine->str[i+1];
    }
    
    char* nouv = realloc(chaine->str, (length(*chaine)-1) * sizeof(char));
    if (nouv) {
        chaine->str = nouv;
    }
    chaine->N --;
