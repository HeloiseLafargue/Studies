...
function Bloc_IDCT2 = IDCT2Rapide(Bloc_DCT2, taille_bloc)
    ...
    for x = 1:taille_bloc: n
        for y = 1:taille_bloc: p
            for u = 0:taille_bloc-1
                for v = 0:taille_bloc-1
                    somme = 0;
                    for i = 0:taille_bloc-1
                        for j = 0:taille_bloc-1
                            somme = somme + (C(i+1) * C(j+1) / 4) * ...
                                Bloc_DCT2(x + i, y + j) * ...
                                cos((2 * u + 1) * i * pi / (2 * taille_bloc)) * ...
                                cos((2 * v + 1) * j * pi / (2 * taille_bloc));
                        end
                    end
                    Bloc_IDCT2(x + u, y + v) = somme;
                end
            end
        end
    end
...
%             for j = 0:taille_bloc-1
%                 for i = 0:taille_bloc-1
%                     somme_x = somme_x + C(i) / 2 * Bloc_DCT2(i, j) * ...
%                         cos((2 * x + 1) * i * pi / (2 * taille_bloc));
%                 end
%                 somme_y = somme_y + C(j) / 2 * somme_x * cos((2 * y + 1) * j * pi / (2 * taille_bloc));
%             end
%             Bloc_IDCT2(x, y) = somme;
%         end
%     end
% end
