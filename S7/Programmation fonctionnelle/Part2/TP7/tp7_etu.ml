...
module FreeFall (F : Frame) =
  struct
    ...
    let run : etat -> etat Flux.t = fun(position0, vitesse0) ->
      let acceleration = Flux.constant(0.,-.g) in
      let vitesse = integre F.dt vitesse0 acceleration in
      let position = integre F.dt position0 vitesse in
...
module Drawing (F : Frame) =
  ...
  (* Exercice 2 *)
  (* Module pour obtenir la simulation du modèle de balle *)
  module F = 
  struct
    let dt = 0.01
    let box_x = (0.,800.)
  ...
  let exo2() =
    ...
    Draw.draw(FF.run (position0,vitesse0))

  (* Exercice 3 *)
  module Exo3 (F: Frame) = struct
    let rec unless flux cond f_cond =
      Tick(lazy(
        ...
        | Some(t, q) -> 
          if cond t then 
            Flux.uncons(f_cond t) (* uncons pour enlever le tick lazy de f_cond *)
  ...
  let contact_x = contact_1d F.box_x
  let contact_y = contact_1d F.box_y

  let rebond_1d box_z z dz =
    if contact_1d box_z z dz then -.dz
    else dz
  let rebond_x = rebond_1d F.box_x
  let rebond_y = rebond_1d F.box_y
  ...
  module  Bouncing (F: Frame) = struct
    ...
    let contact((x,y), (dx,dy) : etat) =
    contact_1d F.box_x x dx || contact_1d F.box_y y dy
    let rebond((x,y), (dx,dy) : etat) : etat =
    ((x,y), (rebond_x x dx, rebond_y y dy))

    let rec run etat0 =
      let module FF = FreeFall(F) in
      E.unless (FF.run etat0) contact (fun etat -> run (rebond etat))
  end

  (* Exercice 6 *)
  let exo6 () =
    let module Bounce = Bouncing(F) in
    let position0 = (300., 400.) in
    let vitesse0 = (15., 25.) in
