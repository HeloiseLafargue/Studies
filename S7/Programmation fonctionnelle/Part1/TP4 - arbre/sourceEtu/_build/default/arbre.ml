...
let rec retrait_arbre lc (Noeud (b, lb)) =
   ...
   | t::q ->
      let arbre_c =
        let l = recherche t lb in
        match l with
        | None   -> failwith("Le mot n'est pas dans l'arbre")
        | Some a -> a in Noeud (b, maj t (retrait_arbre q arbre_c) lb)
 
 
 let arbre_test =
   List.fold_right ajout_arbre
   [['b';'a';'s']; ['b';'a';'t']; ['c';'e']; ['c';'e';'t']]
     (Noeud (false,[]))

 (*let%test _ = retrait_arbre ['b';'a';'s'] arbre_test = 
  let nouv_arbre = retrait_arbre ['b';'a';'s'] arbre_test 
  in not (appartient_arbre ['b';'a';'s'] nouv_arbre)
  && appartient_arbre ['b';'a';'t'] nouv_arbre 
  && appartient_arbre ['c';'e'] nouv_arbre
  && appartient_arbre ['c';'e';'t'] nouv_arbre*)

(******************************************************************************)
(*   fonction de parcours lexicographique                                     *)
...
let rec parcours_arbre (Noeud (b, bs)) =
  let parcours_branche (c,sa) = List.map (fun q -> c::q) (parcours_arbre sa) in
...
let arbre_simple = 
  List.fold_right ajout_arbre[['b';'a';'s']; ['b';'a';'t']]
  (Noeud(false, []))

let arbre_1feuille = 
  List.fold_right ajout_arbre[['b']]
  (Noeud(false, []))