  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    |(FunctionToken)->
      ...
      (inject stream >>=
      accept FunctionToken >>= acceptIdent >>= accept BodyToken >>= parseE)

    (* regle #2 *)
    |(LetToken)->
      (print_endline "let ident = E in E");
      (inject stream >>=
      accept LetToken >>= acceptIdent >>= accept EqualToken >>= parseE >>= accept InToken >>= parseE)

    (* regle #3 *)
    |(RecToken)->
      (print_endline "letrec ident = E in E");
      (inject stream >>=
      accept RecToken >>= acceptIdent >>= accept EqualToken >>= parseE >>= accept InToken >>= parseE)
    ...
    |(IfToken)->
      ...
      (inject stream >>=
      accept IfToken >>= parseE >>= accept ThenToken >>= parseE >>= accept ElseToken >>= parseE)
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    |(LeftParenthesisToken)->
      (print_endline "( E )");
      (inject stream >>=
      accept LeftParenthesisToken >>= parseE >>= accept RightParenthesisToken)
